...
class Image:
    def __init__(self):
        ...
        self.halo_center, self.halo_angmom = self.halo_center_angmom(self.sim, self.halo_num, particle_type=self.particles)

        if GRIDDED:
            import reconstruct_grid
            grid_filename = glob(GRID_ROOT + '*' + self.timestep + '*' + str(CELLS_PER_SIDE) + 'cells.' + self.particles + '.hdf5')[0]
            self.take = reconstruct_grid.reconstruct_indices(grid_filename, self.halo_center, self.radius)
            print('Grid loaded')
        else:
            self.take = None

        self.sim = self._load(self.snapshot, particle_type=self.particles, take=self.take)

        self.image_sim = self.restrict_to_sphere(self.halo_center, self.radius)
        self.center_sim()

        self.rotate_sim(self.parser.sideon)
    ...
    def halo_center_angmom(self, sim, halo_num, particle_type='stars'):
        ...
        if not sum(angmom_vec) or not sum(angmom_vec) == sum(angmom_vec):
            print('Not enough particles to calculate angular momentum, exiting...')
            sys.exit()
    ...
    def rotate_sim(self, sideon):
        ...
        print('Halo rotated')
        self.sim.transform(rotation_matrix)

    def generate_image(self, units='Msol Mpc**-2', cmap='viridis',
                       vmin=1e10, vmax=1e14, **kwargs):
        plt.ion()
        while True:
            if RENDER_STARS:
                pynbody.plot.stars.render(self.image_sim, width=2*self.radius)
            else:
                pynbody.plot.image(self.image_sim, width=2 * self.radius, units=units,
    ...
    def exit_dialog(self):
        if 'y' in input('Exit (y/n): '):
            if 'y' in input('Save (y/n): '):
                plt.savefig(fname=input('Filename: '))
            sys.exit()
        else:
            return False
